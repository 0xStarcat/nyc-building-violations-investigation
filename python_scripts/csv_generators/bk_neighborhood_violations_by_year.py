...
def generate_row(boundary, years_found, buildings_data):
  ...
  for year in sorted(years_found):
    ...
    violations_per_building.append(violations_helpers.calculate_violations_per_building(len(boundary["violations"][year]), violations_helpers.match_boundary_to_building_num(boundary, "neighborhood", buildings_data), year)) if year in boundary["violations"] else violations_per_building.append("0")

  return [boundary["neighborhood"]] + total_violation_by_year + average_violation_per_month + [violations_helpers.match_boundary_to_building_num(boundary, "neighborhood", buildings_data)] + violations_per_building

def write_csv(boundary_list, years_found, buildings_data):
  with open("data/violations_data/csv/bk_neighborhood_violations_by_year.csv", 'w') as outcsv:
    print("writing to CSV")
    writer = csv.writer(outcsv)
    writer.writerow(["neighborhood"] + generate_year_headers(years_found))
    
    for boundary in sorted(boundary_list, key=lambda obj: obj["neighborhood"]):
      if boundary["neighborhood"] != "" and "neighborhood" in boundary:
