...
def seed_building_events(c):
  ...
  event_col6 = 'event_date'


  c.execute('CREATE TABLE IF NOT EXISTS {tn} (id INTEGER PRIMARY KEY AUTOINCREMENT, {col1} INTEGER NOT NULL REFERENCES {ct_table}(id), {col2} INTEGER NOT NULL REFERENCES {n_table}(id), {col3} INTEGER NOT NULL REFERENCES {bldg_table}(id), {col4} TEXT NOT NULL, {col5} INTEGER NOT NULL, {col6} TEXT)'\
    .format(tn=building_events_table, col1=event_col1, col2=event_col2, col3=event_col3, col4=event_col4, col5=event_col5, col6=event_col6, ct_table=census_tracts_seeds.census_tracts_table, n_table=neighborhoods_seeds.neighborhoods_table, bldg_table=buildings_seeds.buildings_table))
